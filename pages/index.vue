<template>
  <div class="flex flex-row items-center justify-center min-h-screen">
    ...
    <div v-if="showAdminForm" class="bg-dark-blue/90 rounded-lg m-60 w-96 h-96 flex flex-col items-center justify-center">
      <form class="flex flex-col items-center justify-center" @submit.prevent="goAdminPage">
        <div class="h-5">
          <p class="text-red-500 text-xs font-bold">{{ errorKeyMessage }}</p>
        </div>
        <VTextField v-model="configKey" type="text" placeholder="enter secret key" class="w-60 h-12 text-black m-2" />
        <VButton
    ...
    <div v-if="!showAdminForm" class="bg-dark-blue/90 rounded-lg m-60 w-96 h-96 flex flex-col items-center justify-center">
      <form class="flex flex-col items-center" @submit.prevent="userLogin">
        ...
        <VTextField v-model="email" type="email" placeholder="email" class="w-60 h-12 text-black m-2" />
        <VTextField v-model="password" type="password" placeholder="password" class="w-60 h-12 text-black m-2" />
        <VButton
          class="w-24 mt-3"
          :is-bold="true"
          :is-rounded="true"
...
<script setup>
  ...
  const switchAdmin = () => {
   ...
   switchLabel.value = showAdminForm.value ? 'login as student' : 'login as admin'
  }
